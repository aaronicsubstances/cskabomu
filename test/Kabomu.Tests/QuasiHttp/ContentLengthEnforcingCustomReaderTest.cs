...
namespace Kabomu.Tests.QuasiHttp
{
    public class ContentLengthEnforcingCustomReaderTest
    {
        ...
        {
            // arrange
            var stream = new RandomizedReadSizeBufferReader(MiscUtils.StringToBytes(srcData));
            var instance = new ContentLengthEnforcingCustomReader(stream,
                contentLength);

            // act
            var actual = MiscUtils.BytesToString(await IOUtils.ReadAllBytes(
                instance));
            ...
            actual = MiscUtils.BytesToString(await IOUtils.ReadAllBytes(
                instance));
            Assert.Equal("", actual);
        }
        ...
        {
            // arrange
            var stream = new MemoryStream(MiscUtils.StringToBytes(srcData));
            var instance = new ContentLengthEnforcingCustomReader(stream,
                contentLength);
